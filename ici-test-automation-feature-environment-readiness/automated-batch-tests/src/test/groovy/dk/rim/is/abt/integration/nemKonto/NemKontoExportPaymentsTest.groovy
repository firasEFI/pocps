...
class NemKontoExportPaymentsTest {

    private static final String JOB_DESCRIPTION = "NemKontoExportPayments"

    private static final String NEMKONTO_PAYMENT = "NEMKONTO_PAYMENT"

    private static final String STATUS = "EXPORTED_BY_INTEGRATION_SUCCESS"
    ...
    void cleanDb() {
        ...
        BatchController.runAndReportIntegrationJob("NemKontoReceiveReceipts")
    }

    private void checkStatusInDb() {
        List<IntegrationFileEntity> files = fileDao.getBy({ entity -> (file.getFilename()).equals(entity.getFilename()) } as Predicate)

        assertThat(files).hasSize(1)
        assertThat(files.get(0).getStatus()).isEqualTo(STATUS)
    }
